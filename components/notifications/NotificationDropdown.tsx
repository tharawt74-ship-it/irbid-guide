...
export function NotificationAvatar({ 
  ...
  const [bizData, setBizData] = useState<{ name?: string; logoUrl?: string } | null>(null);
  const [loading, setLoading] = useState(false);

  const businessId = notif.businessId || (() => {
    const match = notif.link?.match(/\/business\/([a-zA-Z0-9_-]+)/);
    return match ? match[1] : null;
  })();

  useEffect(() => {
    if (notif.businessName || notif.businessLogoUrl) {
      setBizData({
        name: notif.businessName,
        logoUrl: notif.businessLogoUrl
      });
      return;
    }
    ...
    getDoc(doc(db, 'businesses', businessId))
      ...
      .catch((err) => console.warn("Error fetching business for notification avatar:", err))
      .finally(() => {
  ...
  }, [businessId, notif.businessName, notif.businessLogoUrl]);

  const IconComponent = iconConfig.icon;
  const bgClass = iconConfig.bgColor || iconConfig.colorClass || '';

  if (loading) {
    return (
      <div className={cn("flex items-center justify-center shrink-0 shadow-2xs animate-pulse", className, bgClass)}>
  ...
  if (bizData && bizData.logoUrl) {
    return (
      <div className={cn("overflow-hidden shrink-0 shadow-2xs border border-stone-200 bg-white", className)}>
        <img
          src={bizData.logoUrl}
          alt={bizData.name || 'Logo'}
  ...
  if (bizData && bizData.name) {
    const firstLetter = bizData.name.trim().charAt(0);
    return (
      <div className={cn("bg-gradient-to-br from-[#1a4d2e] to-emerald-600 flex items-center justify-center text-white font-black text-lg shadow-inner shrink-0", className)}>
        {firstLetter}
      </div>
    );
  }
  ...
  return (
    <div className={cn("flex items-center justify-center shrink-0 shadow-2xs", className, bgClass)}>
            ...
            {displayedNotifications.length === 0 ? (
              ...
              displayedNotifications.slice(0, 6).map((notif) => {
                ...
                return (
                  ...
                  >
                    {/* Icon Avatar */}
                    <NotificationAvatar notif={notif} iconConfig={iconConfig} />
