  ...
  const fetchData = async () => {
    ...
    try {
      ...
      try {
        ...
        housingsSnap.forEach(d => fetchedHousings.push({ id: d.id, ...d.data() }));
  ...
  const handleMarketingStatusUpdate = async (
    ...
    try {
      ...
      if ((serviceType === 'sponsored' || serviceType === 'homepage_banner') && businessId) {
        const isFeatured = newStatus === 'completed' || newStatus === 'approved';
        await updateDoc(doc(db, 'businesses', businessId), { isFeatured });
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-16" dir="rtl">
      ...
      {activeTab === 'housing' && (
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-[#e5e1da] shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              <h3 className="text-lg font-black text-stone-900">إدارة ونشر السكنات والعقارات</h3>
              <p className="text-xs text-stone-500">إضافة وتوثيق وتعديل سكنات الطلاب والطالبات والشقق السكنية في إربد</p>
            </div>
            <button
              onClick={() => {
                setEditingHousing(null);
            ...
            >
              <Plus className="h-4 w-4" />
              <span>{isHousingFormOpen ? 'إغلاق النموذج' : 'إضافة إعلان عقار جديد'}</span>
            </button>
          </div>
          ...
          {isHousingFormOpen && (
            <form onSubmit={handleSaveHousing} className="bg-white p-6 rounded-3xl border border-emerald-500/20 shadow-md space-y-4">
              <h4 className="font-black text-sm text-[#1a4d2e] pb-2 border-b border-stone-100 flex items-center gap-2">
                ...
                <span>{editingHousing ? 'تعديل الإعلان العقاري' : 'نشر إعلان عقاري جديد'}</span>
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
              ...
              <div className="pt-2 flex justify-end gap-2">
                ...
                <button
                  ...
                  className="bg-[#1a4d2e] hover:bg-[#143e25] text-white px-5 py-2.5 rounded-xl text-xs font-black"
                >
                  {editingHousing ? 'حفظ التعديلات' : 'نشر الإعلان الآن'}
                </button>
              </div>
            </form>
          )}
          ...
          <div className="grid gap-4">
            {housings.length === 0 ? (
              <div className="bg-white py-12 rounded-3xl border border-[#e5e1da] text-center text-xs text-stone-500">
                لا توجد عقارات أو سكنات منشورة حالياً في قاعدة البيانات
              </div>
            ) : (
              housings.map(item => (
                <div key={item.id} className="bg-white p-5 rounded-3xl border border-[#e5e1da] shadow-xs flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                  <div className="space-y-1.5 flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-[#1a4d2e] font-black text-xs px-2.5 py-0.5 bg-emerald-50 rounded-full">
                    ...
                    <p className="text-xs text-stone-600 line-clamp-1">{item.description || item.location}</p>
                    <div className="flex gap-4 text-[10px] text-stone-400">
                      <span>المالك: {item.ownerName || 'غير محدد'}</span>
                      <span>الهاتف: {item.contactPhone}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 w-full md:w-auto shrink-0 justify-end pt-2 md:pt-0">
                    <button
                      onClick={() => handleToggleHousingVerified(item)}
                      className={`p-2 rounded-xl text-xs transition-colors cursor-pointer ${item.isVerified ? 'bg-blue-50 text-blue-600' : 'bg-stone-50 text-stone-400 hover:text-blue-600'}`}
                      title={item.isVerified ? 'إلغاء وسم موثق' : 'توثيق واعتماد الإعلان'}
                    >
