...
import { BUSINESS_CATEGORIES, MainCategory } from '../lib/categories';
...
export function Home() {
  ...
  const [bannerBusinesses, setBannerBusinesses] = useState<Business[]>([]);
  ...
  useEffect(() => {
    async function fetchBusinesses() {
      ...
      try {
        ...
        // Fetch approved homepage banner marketing requests
        try {
          const mktQuery = query(collection(db, 'marketingRequests'));
          const mktSnap = await getDocs(mktQuery);
          const approvedBannerIds: string[] = [];
          mktSnap.forEach((docSnap) => {
            const data = docSnap.data();
            if (
              data.serviceType === 'homepage_banner' &&
              (data.status === 'completed' || data.status === 'approved') &&
              data.businessId
            ) {
              approvedBannerIds.push(data.businessId);
            }
          });

          // Match businesses that have approved homepage_banner marketing request or marked as featured by Admin
          const approvedBanners = fetchedBusinesses.filter(b => approvedBannerIds.includes(b.id) || b.isFeatured);
          setBannerBusinesses(approvedBanners);
        } catch (mktErr) {
          console.error("Error fetching marketing banner requests:", mktErr);
          setBannerBusinesses([]);
        }
      } catch (err) {
  ...
  const mainCategories = Object.keys(BUSINESS_CATEGORIES) as MainCategory[];
  ...
  const filteredBusinesses = businesses.filter(b => {
    ...
    if (categoryFilter) {
      const validSubCats = BUSINESS_CATEGORIES[categoryFilter] || [];
  ...
  if (searchTerm) {
    ...
    if (activeTab === 'featured') {
      displayedBusinesses = displayedBusinesses.filter(b => b.isFeatured);
  ...
  return (
    <div className="space-y-6 md:space-y-10">
      ...
      />
      {/* Hero Section */}
      <div className="bg-[#1a4d2e] rounded-3xl md:rounded-[32px] p-6 md:p-16 text-white flex flex-col items-center text-center relative overflow-hidden shadow-xl shadow-[#1a4d2e]/10">
        ...
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#ff9f1c] rounded-full blur-[100px] opacity-20 -translate-y-1/2 translate-x-1/3"></div>
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#ffffff] rounded-full blur-[120px] opacity-10 translate-y-1/2 -translate-x-1/3"></div>
        
        <div className="relative z-10 w-full max-w-3xl mx-auto space-y-4 md:space-y-6">
          <span className="inline-block py-1 px-3 md:py-1.5 md:px-4 rounded-full bg-white/10 border border-white/20 text-white/90 text-xs md:text-sm font-bold backdrop-blur-sm mb-1 md:mb-2">
          ...
          <p className="text-sm md:text-xl text-white/80 max-w-2xl mx-auto leading-relaxed px-2">
          ...
          <div className="mt-6 md:mt-10 w-full max-w-2xl mx-auto relative group">
            <div className="absolute inset-y-0 right-0 pr-4 md:pr-5 flex items-center pointer-events-none text-stone-400 group-focus-within:text-[#ff9f1c] transition-colors">
              <Search className="h-5 w-5 md:h-6 md:w-6" />
            </div>
            <input
              type="text"
              className="block w-full pr-12 pl-10 py-3.5 md:pr-14 md:pl-6 md:py-5 border-none rounded-2xl md:rounded-[24px] leading-5 bg-white/95 backdrop-blur-md text-[#2d2a26] placeholder-stone-400 focus:outline-none focus:ring-4 focus:ring-[#ff9f1c]/40 shadow-xl font-bold text-base md:text-lg transition-all"
            ...
            {searchTerm && (
              <button 
                ...
                className="absolute left-3 top-1/2 -translate-y-1/2 bg-stone-100 hover:bg-stone-200 text-stone-500 rounded-full p-1.5 transition-colors cursor-pointer"
              >
                ✕
              </button>
            )}
          ...
          {searchTerm && (
            <div className="mt-2 inline-flex items-center gap-1.5 text-white/95 text-[10px] md:text-xs font-black bg-emerald-950/45 backdrop-blur-md py-1.5 px-3 md:py-2 md:px-4 rounded-xl border border-emerald-500/35 text-right">
      ...
      {!isFiltering && bannerBusinesses.length > 0 && (
        <BannerSlideshow businesses={bannerBusinesses} />
      )}
      ...
      {mainCategories.length > 0 && (
        <div className="flex flex-col space-y-4">
          ...
          {categoryFilter && BUSINESS_CATEGORIES[categoryFilter as MainCategory] && (
            <div className="relative">
              ...
              <div className="flex overflow-x-auto gap-2.5 pb-3 mt-4 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide snap-x" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                ...
                {BUSINESS_CATEGORIES[categoryFilter as MainCategory].map((subCat) => (
                  <button
                    key={subCat}
